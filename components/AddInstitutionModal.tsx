...
interface AddInstitutionModalProps {
  onClose: () => void;
  onAddInstitution: (data: Omit<Institution, 'id'>) => void;
}

const AddInstitutionModal: React.FC<AddInstitutionModalProps> = ({ onClose, onAddInstitution }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-lg relative animate-fade-in-up">
        ...
        <h2 className="text-2xl font-bold mb-6">Add New Institution</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-neutral-600">Institution Name</label>
            <input
              id="name"
              type="text"
              placeholder="e.g., Apex University of Science"
              value={name}
